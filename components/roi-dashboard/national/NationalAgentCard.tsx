import { Network, Truck, FileText, ShieldAlert } from "lucide-react";
import { type NatAgent, formatNatMetric, useNational } from "@/lib/roi-dashboard/national/store";
...
const iconMap = { apex: Network, dispatch: Truck, report: FileText, watch: ShieldAlert };

export function NationalAgentCard({ agent }: { agent: NatAgent }) {
  const Icon = iconMap[agent.id];
  const { setSelectedAgent } = useNational();
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex items-baseline gap-2">
              <span className="font-display text-2xl font-bold tracking-tight">
                <AnimatedNumber value={agent.metricValue} format={(n) => formatNatMetric(n, agent.metricFormat)} />
              </span>
