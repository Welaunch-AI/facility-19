import { Route as RouteIcon, Wrench, MessageSquare, Receipt } from "lucide-react";
import { type FieldAgent, formatFieldMetric, useField } from "@/lib/roi-dashboard/field/store";
...
const iconMap = { route: RouteIcon, tech: Wrench, client: MessageSquare, invoice: Receipt };

export function FieldAgentCard({ agent }: { agent: FieldAgent }) {
  const Icon = iconMap[agent.id];
  const { setSelectedAgent } = useField();
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex items-baseline gap-2">
              <span className="font-display text-2xl font-bold tracking-tight">
                <AnimatedNumber value={agent.metricValue} format={(n) => formatFieldMetric(n, agent.metricFormat)} />
              </span>
